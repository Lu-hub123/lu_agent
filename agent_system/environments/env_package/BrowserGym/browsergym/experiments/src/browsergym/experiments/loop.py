...
from agent_system.environments.env_package.BrowserGym.browsergym.core.src.browsergym.core.action.parsers import highlevel_action_parser
from agent_system.environments.env_package.BrowserGym.browsergym.core.src.browsergym.core.chat import Chat
...
class EnvArgs(DataClassJsonMixin):
    ...
    def make_env(
        ...
        # print("!!!!!!!!!!!!!![DEBUG] Final headless setting:", self.headless)
...
class StepInfo:
    ...
    def from_reset(self, env: gym.Env, seed: int, obs_preprocessor: callable):
        t = self.profiling
        t.env_start = time.time()
        self.obs, env_info = env.reset(seed=seed) #调用reset的时候就会创建新任务对象，根据task返回的goal设置对话界面内容，返回obs和info
...
class ExpArgs:
    ...
    def run(self):
        ...
        try:
            ...
            # env, val_envs = make_envs(config) #TODO
            ...
            while not step_info.is_done:  # set a limit
                ...
                # action = "step_info.from_action(agent)"           #然后获取action，这一步里面才用到了action，后续可以了解到底是怎么用到了agent去获得action
                ...
                step_info.from_step(env, action, obs_preprocessor=agent.obs_preprocessor)#进行step，执行一次
                # Agent.get_action(obs) → 输出 action（字符串）→ env.step(action) → 执行这个动作 → 返回 obs、reward、done 等
